...
import com.lukxtech.powertrain.enginefamily.abstracts.exceptions.*;

/**
 * <p>An generic abstract builder for all the Engines to inherit from</p>
 * <p>This class greatly reduces boilerplate code for creating engine models</p>
 *
...
public abstract class AbstractEngineBuilder<T extends AbstractEngineModel, B extends AbstractEngineBuilder<T, B>> {
    ...
    public void validate_initial() throws BadModelNameException, BadModelNumberException, BadDisplacementValueException, BadPowerValueException, BadRPMValueException {
        if (modelNo == null || modelNo.isBlank())
            throw new BadModelNumberException("BAD MODEL NUMBER! GOT: " + modelNo);
        if (modelName == null || modelName.isBlank())
            throw new BadModelNameException("BAD MODEL NAME! GOT: " + modelName);
        if (displacement <= 0) throw new BadDisplacementValueException("BAD DISPLACEMENT VALUE! GOT: " + displacement);
        if (minPower < 0)
            throw new BadPowerValueException("BAD MINIMUM POWER! GOT: " + minPower + " ALSO, MAX POWER: " + maxPower);
        if ((maxPower <= minPower) || (maxPower < 0) || (maxPower == minPower))
            throw new BadPowerValueException("BAD MAX POWER! GOT: " + maxPower + " ALSO, MIN POWER: " + minPower);
